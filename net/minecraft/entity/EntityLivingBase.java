...
public abstract class EntityLivingBase extends Entity
{
    ...
    private int getArmSwingAnimationEnd()
    {
    	...
    	final int speed = animations.isEnabled() ? animations.swingSpeed.getValue() : 6;
    ...
    protected void jump()
    {
    	final EventJump event = new EventJump(this.getJumpUpwardsMotion());
            ...
            if (!this.isInWater() || this instanceof EntityPlayer && ((EntityPlayer)this).capabilities.isFlying)
            {
                if (!this.isInLava() || this instanceof EntityPlayer && ((EntityPlayer)this).capabilities.isFlying)
                {
                    ...
                    if (this.isOnLadder())
                    {
                        ...
                        final EventMoveOnLadder event = new EventMoveOnLadder(-0.15d);
                        Client.INSTANCE.getEventManager().post(event);
                        if (!event.isCancelled() && this.motionY < event.getLadderSpeed())
                        {
                            this.motionY = event.getLadderSpeed();
                        }
