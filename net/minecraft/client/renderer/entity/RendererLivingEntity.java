...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T>
{
        ...
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, new Object[] {entity, this, Double.valueOf(x), Double.valueOf(y), Double.valueOf(z)}))
        {
            ...
            try
            {
                ...
                if (entity instanceof EntityPlayerSP){
                    ...
                    if (player.playerEvent.isRotating()) {
	                	
	                    f7 = this.interpolateRotation(player.playerEvent.getPrevPitch(), player.playerEvent.getPitch(), partialTicks);
                    }
                }
                this.renderLivingAt(entity, x, y, z);
                f3 = this.handleRotationFloat(entity, partialTicks);
                this.rotateCorpse(entity, f3, f, partialTicks);
                ...
                if (entity instanceof EntityPlayer) {
                    final EventRenderEntity event = new EventRenderEntity(entity, true, f6, f5, f3, f2, f7, f, 0.0625F);
                ...
                this.mainModel.setRotationAngles(f6, f5, f3, f2, f7, 0.0625F, entity);
                ...
                if (this.renderOutlines)
                {
                    ...
                    this.renderModel(entity, f6, f5, f3, f2, f7, 0.0625F);
                ...
                else
                {
                    ...
                    if (EmissiveTextures.isActive())
                    {
                        if (EmissiveTextures.hasEmissive())
                        {
                            ...
                            GlStateManager.pushMatrix();
                            this.renderModel(entity, f6, f5, f3, f2, f7, f4);
                            GlStateManager.popMatrix();
                    ...
                    {
                        this.renderLayers(entity, f6, f5, partialTicks, f3, f2, f7, 0.0625F);
                    }
                }
            ...
            final EventRenderEntity event = new EventRenderEntity(entity, false);
            Client.INSTANCE.getEventManager().post(event);
    ...
    protected boolean setScoreTeamColor(T entityLivingBaseIn)
    {
        ...
        if (entityLivingBaseIn instanceof EntityPlayer)
        {
            ...
            {
                String s = VanillaFontRenderer.getFormatFromString(scoreplayerteam.getColorPrefix());
     ...
     */
    public void renderModel(T entitylivingbaseIn, float p_77036_2_, float p_77036_3_, float p_77036_4_, float p_77036_5_, float p_77036_6_, float p_77036_7_) {
    	final Chams chams = (Chams) Client.INSTANCE.getModuleManager().getModule("chams");
        final OutlineESP outline = (OutlineESP) Client.INSTANCE.getModuleManager().getModule("outlineesp");
    	final int rgb = RenderUtil.getRainbow(6000, -15, 0.8f);
    	boolean flag = !(entitylivingbaseIn.isInvisible() && !((outline.invisibles.isEnabled() && outline.isEnabled()) || (chams.invisibles.isEnabled() && chams.isEnabled())));
    	boolean flag1 = !flag && !(entitylivingbaseIn.isInvisibleToPlayer(Minecraft.getMinecraft().thePlayer) && !((outline.invisibles.isEnabled() && outline.isEnabled()) || (chams.invisibles.isEnabled() && chams.isEnabled())));
        if (flag || flag1) {
            ...
            if (outline.isEnabled()) {
                GL11.glPushMatrix();
                GlStateManager.depthMask(true);
                if (outline.isValid(entitylivingbaseIn)) {
                    if (Minecraft.getMinecraft().theWorld != null) {
                    	final Color c = new Color(outline.outlineColor.getValue());
                    	int r = c.getRed();
                    	int g = c.getGreen();
                    	int b = c.getBlue();
                    	int color = new Color(r, g, b, outline.alpha.getValue()).getRGB();
                    	boolean rainbow = outline.rainbow.isEnabled();
                    	if (!rainbow) {
                    		RenderUtil.glColor(color);
                    	} else {
                    		RenderUtil.glColor(rgb);
                    	}
            ...
            if (chams.isEnabled() && chams.isValid(entitylivingbaseIn)) {
                if (chams.colored.isEnabled()) {
                    ...
                    final Color hC = new Color(chams.hiddenColor.getValue());
                    int hiddenRed = hC.getRed();
                    int hiddenGreen = hC.getGreen();
                    int hiddenBlue = hC.getBlue();
                    final Color visibleColor = new Color(hiddenRed, hiddenGreen, hiddenBlue, chams.alpha.getValue());
                    RenderUtil.glColor(visibleColor.getRGB());
                	boolean rainbow = chams.rainbow.isEnabled();

                    this.mainModel.render(entitylivingbaseIn, p_77036_2_, p_77036_3_, p_77036_4_, p_77036_5_, p_77036_6_, p_77036_7_);
                    
                    GL11.glEnable(GL11.GL_DEPTH_TEST);
                    GL11.glDepthMask(true);
                    if (chams.shadows.isEnabled()) {
                    	GL11.glEnable(GL11.GL_LIGHTING);
                    }
                    
                    final Color vC = new Color(chams.visibleColor.getValue());
                    int visibleRed = vC.getRed();
                    int visibleGreen = vC.getGreen();
                    int visibleBlue = vC.getBlue();
                    
                    if (!rainbow) {             	
                    	RenderUtil.glColor(new Color(visibleRed, visibleGreen, visibleBlue, chams.alpha.getValue()).getRGB());
                    } else {
                    	RenderUtil.glColor(rgb);
                    }
        ...
        if (!Reflector.RenderLivingEvent_Specials_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Specials_Pre_Constructor, new Object[] {entity, this, Double.valueOf(x), Double.valueOf(y), Double.valueOf(z)}))
        {
            if (this.canRenderName(entity))
            {
                ...
                if (d0 < (double)(f * f))
                {
                    ...
                    {
                        VanillaFontRenderer fontrenderer = this.getFontRendererFromRenderManager();
                        GlStateManager.pushMatrix();
