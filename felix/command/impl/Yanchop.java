...
public class Yanchop extends Command {
    ...
    @Override
    public void executeCommand(String[] args) {//your command should call this function
        if(!threadRunning) {
            startThread();
        ...
        if(args[0].startsWith("key=")) {
            key = args[0].substring(4, args[0].length());
            sendMsg("key updated");
        ...
        //do not edit this
        if(args[0].equals("post=false")) {postMode = false;sendMsg("Post mode set to false");return;}
        if(args[0].equals("post=true")) {postMode = true;sendMsg("Post mode set to true, you will now post the search results in public chat");return;}
        if(args[0].equals("bypass=true")) {bypassFilter = true;sendMsg("Chat bypass mode set to true");return;}
        if(args[0].equals("bypass=false")) {bypassFilter = false;sendMsg("Chat bypass mode set to false");return;}
        if(args[0].startsWith("!check") || args[0].startsWith("!status")) {if(key.length() > 8) checkStatus(); else sendMsg("Please activate a key by doing key=(key)");return;}
