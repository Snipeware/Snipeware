...
public class Nametags extends Module {
    ...
    public Nametags() {
        super("Nametags", 0, ModuleCategory.VISUALS);
    ...
    @Handler
    public void onRender2D(final EventRender3D event) {
        mc.theWorld.getLoadedEntityList().forEach(ent -> {
            if (ent instanceof EntityLivingBase) {
                EntityLivingBase entity = (EntityLivingBase) ent;
                if (isValid(entity)) {
                    float x = (float) (entity.lastTickPosX + (entity.posX - entity.lastTickPosX) * event.getPartialTicks() - mc.getRenderManager().getRenderPosX());
                    float y = (float) (entity.lastTickPosY + (entity.posY - entity.lastTickPosY) * event.getPartialTicks() - mc.getRenderManager().getRenderPosY()) - 1.75f + entity.height + (0.015f * mc.thePlayer.getDistanceToEntity(entity));
                    float z = (float) (entity.lastTickPosZ + (entity.posZ - entity.lastTickPosZ) * event.getPartialTicks() - mc.getRenderManager().getRenderPosZ());
